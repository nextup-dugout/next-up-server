...
@DisplayName("SeasonFieldingStats.applyFieldCorrection")
class SeasonFieldingStatsApplyFieldCorrectionTest {
    ...
    fun setup() {
        ...
        stats = SeasonFieldingStats.create(player, 2026)
    ...
    inner class ErrorCases {
        ...
        fun `유효하지 않은 필드명이면 IllegalArgumentException이 발생한다`() {
            assertThatThrownBy {
                ...
                .hasMessageContaining("유효하지 않은 시즌 수비 통계 필드입니다")
        }

        @Test
        fun `gamesPlayed는 지원하지 않으므로 IllegalArgumentException이 발생한다`() {
            assertThatThrownBy {
