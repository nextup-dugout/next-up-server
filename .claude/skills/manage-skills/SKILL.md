...
| 모듈 | 파일 |
|------|------|
| nextup-core | `entity/Player.kt`, `service/PlayerService.kt` |
| nextup-infrastructure | `repository/PlayerRepositoryImpl.kt` |
| nextup-api | `controller/PlayerController.kt`, `dto/PlayerResponse.kt` |
| (루트) | `build.gradle.kts` |
```
...
| 스킬 | 트리거 파일 (변경된 파일) | 액션 |
|------|--------------------------|------|
| verify-entity-leak | `PlayerController.kt`, `PlayerResponse.kt` | CHECK |
| verify-dependency | `build.gradle.kts` | CHECK |
| (스킬 없음) | `application.yml` | UNCOVERED |
```
...
3. **커버되지 않은 새 패턴** — 변경된 파일을 읽고 스킬이 검사하지 않는 새로운 규칙, 설정, 패턴을 식별합니다. 확인 사항:
   - 새로운 타입 정의, enum, data class, sealed class
   - 새로운 Entity, Repository, Service, Controller 등록
   - 새로운 모듈 의존성 또는 Gradle 설정
...
| 스킬 | 갭 유형 | 상세 |
|------|---------|------|
| verify-entity-leak | 파일 누락 | `dto/NewResponse.kt`가 Related Files에 없음 |
| verify-dependency | 새 패턴 | 새 모듈 의존성이 검사되지 않음 |
```
...
**결정: 기존 스킬 UPDATE** (N개)
- `verify-entity-leak` — 누락된 파일 참조 2개 추가, 탐지 패턴 업데이트
- `verify-dependency` — 새 모듈 의존성에 대한 탐지 명령어 업데이트

**결정: 새 스킬 CREATE** (M개)
- 새 스킬 필요 — <패턴 설명> 커버 (X개 미커버 파일)

**액션 불필요:**
- `build.gradle.kts` — 설정 파일, 면제
- `README.md` — 문서, 면제
```
...
|------|---------|
| ... 기존 항목 ... |
| `nextup-api/src/.../dto/NewResponse.kt` | 새 응답 DTO |
```
...
```bash
grep -rn "pattern" nextup-api/src/
```
...
2. **사용자에게 스킬 이름 확인** — `AskUserQuestion`을 사용합니다:
   ...
   **이름 규칙:**
   - 이름은 반드시 `verify-`로 시작해야 합니다 (예: `verify-entity-leak`, `verify-dependency`, `verify-api-response`)
...
1. **Lock 파일 및 생성된 파일** — `gradle.lockfile`, 자동 생성된 마이그레이션 파일, 빌드 출력물은 스킬 커버리지가 불필요
2. **일회성 설정 변경** — `build.gradle.kts`의 버전 범프, 린터/포매터 설정의 사소한 변경은 새 스킬이 불필요
3. **문서 파일** — `README.md`, `CHANGELOG.md`, `LICENSE` 등은 검증이 필요한 코드 패턴이 아님
4. **테스트 픽스처 파일** — 테스트 픽스처로 사용되는 디렉토리의 파일(예: `fixtures/`, `test-data/`)은 프로덕션 코드가 아님
...
7. **벤더/서드파티 코드** — 외부 라이브러리 디렉토리의 파일은 외부 규칙을 따름
8. **CI/CD 설정** — `.github/`, `Dockerfile` 등은 인프라이며, 검증 스킬이 필요한 애플리케이션 패턴이 아님
9. **Flyway 마이그레이션** — `db/migration/` 내의 SQL 파일은 한번 적용되면 수정 불가한 파일이므로 패턴 검증 대상이 아님
