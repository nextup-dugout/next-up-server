...
name: pre-pr-gate
description: |
  PR 생성 전 필수 품질 게이트. ktlint 포맷팅, Gradle 빌드+테스트, Jacoco 커버리지 검증, verify 스킬을
  기계적으로 실행하여 모든 항목 통과 시에만 PR 생성을 허용한다.
  반드시 이 스킬을 사용하라 — 다음 상황에서:
  (1) PR 올리기 전 검증: "PR 올려도 되는지 체크해줘", "빌드 통과하는지 확인", "CI 사전 검증"
  (2) 품질 게이트 통합 실행: "ktlint, 테스트, 커버리지 한번에 확인", "빌드 돌려보고 PR 만들어줘"
  (3) CI 실패 후 재검증: "커버리지 올려서 다시 게이트 통과시켜줘", "ktlint 고쳤는데 다시 확인"
  (4) issue-progress Phase 5에서 자동 호출 (에이전트 내부 사용)
  단순 빌드 실행("./gradlew build 돌려줘")이나 단일 verify 스킬 확인, 코드 리뷰(/review)에는 사용하지 않는다.
user-invocable: true
