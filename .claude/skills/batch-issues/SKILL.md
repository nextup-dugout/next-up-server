...
name: batch-issues
description: |
  열린 GitHub 이슈를 자동으로 수집하고, 이슈 간 의존관계를 분석하여 실행 순서(Wave)를 결정한 뒤,
  Wave별로 병렬 구현 파이프라인(issue-progress)을 실행하는 배치 오케스트레이터.
  각 이슈의 유형(entity/service/api/bug)과 관련 스킬을 자동 판단하고,
  Entity 생성 이슈가 먼저, 그것에 의존하는 API 이슈가 나중에 실행되도록 순서를 조율한다.
  "이슈 다 진행해줘", "열린 이슈 전부 해줘", "이슈 목록 진행", "남은 이슈 처리해줘", "batch issues", "이슈 일괄 처리",
  "이슈 전부 작업", "밀린 이슈 다 해줘", "#123, #456 해줘", "#440 #441 #442 진행해줘" 등
  여러 이슈를 한꺼번에 처리하라는 요청에 반드시 이 스킬을 사용하라.
  단일 이슈 요청("#123 해줘")에는 issue-progress를 사용하고, 이 스킬은 2개 이상의 이슈를 병렬 처리할 때 사용한다.
user-invocable: true
