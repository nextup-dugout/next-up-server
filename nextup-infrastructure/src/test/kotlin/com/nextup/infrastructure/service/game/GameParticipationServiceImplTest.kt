...
@DisplayName("GameParticipationServiceImpl")
class GameParticipationServiceImplTest {
    ...
    private lateinit var service: GameParticipationServiceImpl
    ...
    inner class Vote {
        ...
        fun `should change existing vote`() {
            // given
            val existingVote = GameParticipation.createForGame(game, member)
    ...
    inner class GetVoteSummary {
        ...
        fun `should get vote summary with counts`() {
            ...
            val votes =
                listOf(
                    GameParticipation.createForGame(game, member).apply { vote(AttendanceStatus.ATTENDING) },
                    GameParticipation.createForGame(game, member).apply { vote(AttendanceStatus.ATTENDING) },
                    GameParticipation.createForGame(game, member).apply { vote(AttendanceStatus.ABSENT) },
    ...
    inner class GetNonVoters {
        ...
        fun `should get non-voters list`() {
            ...
            val votes =
                listOf(
                    GameParticipation.createForGame(game, member1).apply { vote(AttendanceStatus.ATTENDING) },
                    GameParticipation.createForGame(game, member2), // UNDECIDED
                    GameParticipation.createForGame(game, member3), // UNDECIDED
                )
    ...
    inner class CreateVotesForGame {
        ...
        fun `should create votes for all active members`() {
            ...
            every { attendanceVoteRepository.saveAll(any<List<GameParticipation>>()) } answers { firstArg() }
            ...
            verify { attendanceVoteRepository.saveAll(any<List<GameParticipation>>()) }
        }
